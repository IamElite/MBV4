...
from ERAVIBES import era, app, db, lang, queue, tg, yt
from ERAVIBES.helpers import admin_check, buttons, can_manage_vc
...
async def _controls(_, query: types.CallbackQuery):
    ...
    if action == "pause":
        ...
        return await era.play_media(chat_id, msg, media)

    elif action == "replay":
        media = queue.get_current(chat_id)
        media.user = user
        await era.replay(chat_id)
        status = query.lang["replayed"]
        reply = query.lang["play_replayed"].format(user)

    elif action == "stop":
        await era.stop(chat_id)
