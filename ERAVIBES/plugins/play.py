...
from ERAVIBES import era, app, config, db, lang, queue, tg, yt
from ERAVIBES.helpers import buttons, utils
from ERAVIBES.helpers._play import checkUB
...
async def play_hndlr(
    ...
    if not file.file_path:
        try:
            ...
            await era.stop(m.chat.id)
            return await sent.edit_text(
                m.lang["error_no_file"].format(config.SUPPORT_CHAT)
            )

    await era.play_media(chat_id=m.chat.id, message=sent, media=file)
